...
function(_, Blueprint, Schema, Tag, Pipeline, merge) {
  ...
  var Form = function(schema, values, options) {
    ...
    this.build = function(schema, prefix_id, traverse) {
      ...
      // Build it.
      return this.build_node(prefix_id, section, schema, traverse);
    };

    // Build an individual form element.
    this.build_tag = function(id, name, schema) {

      var tag = new Tag(id, name, '', schema);
    ...
    this.build_node = function(prefix_id, section, schema, traverse) {
      ...
      _.each(schema,function(source, name) {
        ...
        if(!data.tag) {
          ...
          node.append(form.build_tag(prefix_id+name, name, data));
        }
