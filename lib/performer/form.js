    ...
    var Form = function(schema, options) {
      ...
      this.build = function(section, traverse, recursing) {
        ...
        _.each(context._fields,function(data, key) {
          ...
          // If object has a "_fields" key (and we are traversing), build the subsection
          if(data._fields && traverse === true) {

            output.append(form.build(data, true, true));
          } else {

            // Otherwise, build the tag
            tag = new Tag(key, data, form.options.type_system);
            form.options.pipeline.tag(tag, tag);
            output.append(tag.read());
          }

        });

        // When recursion bottoms out, pass output through group
        // pipeline for final transformation.
        if(!recursing) {
          form.options.pipeline.group(output, output);
        }
